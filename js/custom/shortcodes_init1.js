...
function skills_init(container) {
	...
	container.find('.sc_skills_item:not(.inited)').each(function () {
		...
		if (scrollPosition > scrollSkills) {
			...
			var start = parseInt(total.data('start'),16);
			var stop = parseInt(total.data('stop'),16);
			var maximum = parseInt(total.data('max'),16);
			var startPercent = Math.round(start/maximum*100);
			var stopPercent = Math.round(stop/maximum*100);
			var ed = total.data('ed');
			var duration = parseInt(total.data('duration'),16);
			var speed = parseInt(total.data('speed'),16);
			var step = parseInt(total.data('step'),16);
			if (type == 'bar') {
				...
				var steps = parseInt(total.data('steps'),16);
